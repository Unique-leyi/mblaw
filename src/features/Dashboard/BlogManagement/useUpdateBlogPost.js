...
import { updateBlogPost as updateBlogPostApi } from "../../../services/apiBlogPosts";
...
export function useUpdateBlogPost() {
    ...
    const { mutate: updateBlogPost, isPending: isUpdating } = useMutation({
        ...
        mutationFn: ({ id, data }) => updateBlogPostApi(id, data),
        ...
        onError: (error) => {
            if (error?.data?.success === false) {
                ...
                toast({
                    ...
                    description: error?.data?.message || "Failed to update blog post. Please try again.",
                    status: "error",
                    duration: 5000,
                    isClosable: true,
                    position: "top-right",
    ...
    return {
        updateBlogPost: (id, data) => updateBlogPost({ id, data }),
        isUpdating,
    };
}
