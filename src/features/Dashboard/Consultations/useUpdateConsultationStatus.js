...
import { updateConsultationStatus as updateConsultationStatusApi } from "../../../services/apiConsultations";
...
export function useUpdateConsultationStatus() {
    ...
    const { mutate: updateStatus, isPending: isUpdating } = useMutation({
        mutationKey: ["updateConsultationStatus"],
        mutationFn: ({ id, status }) => updateConsultationStatusApi(id, status),
        ...
        onError: (error) => {
            if (error?.data?.success === false) {
                ...
                toast({
                    ...
                    description: error?.data?.message || "Failed to update status. Please try again.",
                    status: "error",
                    duration: 5000,
                    isClosable: true,
                    position: "top-right",
