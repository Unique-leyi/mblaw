...
export function useDeleteConsultation() {
    ...
    const { mutate: deleteConsultation, isPending: isDeleting } = useMutation({
        mutationKey: ["deleteConsultation"],
        mutationFn: (id) => deleteConsultationApi(id),
        ...
        onError: (error) => {
            if (error?.data?.success === false) {
                ...
                toast({
                    ...
                    description: error?.data?.message || "Failed to delete consultation. Please try again.",
                    status: "error",
                    duration: 5000,
                    isClosable: true,
                    position: "top-right",
